...
RESULTS_PATH = ROOT / "evaluation" / "results.jsonl"
SUMMARY_PATH = ROOT / "evaluation" / "summary.json"


def ensure_ragas_importable() -> None:
    """Compatibility shim for the installed RAGAS 0.4.3 package."""
    try:
    ...
    # Retry the import after adding the missing module.
    import ragas  # noqa: F401
...
def build_ragas_metrics() -> list[Any]:
    ...
    m_context_precision = deepcopy(context_precision)
...
def aggregate_scores(result: Any) -> dict[str, float]:
    if hasattr(result, "_repr_dict"):
        return {key: float(value) for key, value in result._repr_dict.items() if value is not None}
...
def run_evaluation() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for item in rows:
        ...
        generated_answer = safe_text(generated.get("answer", "") if isinstance(generated, dict) else "")
        retrieved_contexts = []
        ...
        if unanswerable:
            ...
            record["metric_scores"] = {"faithfulness": None, "answer_relevancy": None, "context_precision": None, "context_recall": None}
    ...
    if answerable_dataset:
        ...
        for item in result_rows:
            if item.get("evaluated"):
                item["metric_scores"] = {"faithfulness": None, "answer_relevancy": None, "context_precision": None, "context_recall": None}
                # Match the score by question id using the dataset order.
                matches = [row for row in answerable_dataset if row["user_input"] == item["question"]]
                if not matches:
                    continue
                ...
                if hasattr(result, "scores"):
                    ...
                    item["metric_scores"] = {
                        key: value for key, value in score_row.items() if isinstance(value, (int, float))
                    }
        summary["metrics"] = {
            ...
            "context_precision": float(summary["metrics"].get("context_precision", 0.0) or 0.0),
...
def main() -> None:
    ...
    except Exception as exc:  # pragma: no cover - console failure handling
